...
void testConditionalUse() {
  ...
  obj.m_ptr = &obj;
  lfort_analyzer_eval(obj.m_ptr); // expected-warning{{TRUE}}
  lfort_analyzer_eval(&A::m_ptr); // expected-warning{{TRUE}}
  lfort_analyzer_eval(obj); // expected-warning{{TRUE}}

  obj.m_ptr = 0;
  lfort_analyzer_eval(obj.m_ptr); // expected-warning{{FALSE}}
  lfort_analyzer_eval(A::MemberPointer(0)); // expected-warning{{FALSE}}
  lfort_analyzer_eval(obj); // expected-warning{{FALSE}}
}
...
bool testDereferencing() {
  ...
  lfort_analyzer_eval(obj.*member == 0); // expected-warning{{UNKNOWN}}
