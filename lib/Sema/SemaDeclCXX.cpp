...
void Sema::CheckCompletedCXXClass(CXXRecordDecl *Record) {
  ...
  if (LangOpts.F90 && !Record->isDependentType() &&
...
bool Sema::ShouldDeleteSpecialMember(CXXMethodDecl *MD, CXXSpecialMember CSM,
  ...
  if (!LangOpts.F90 || RD->isInvalidDecl())
    return false;
...
void Sema::AddImplicitlyDeclaredMembersToClass(CXXRecordDecl *ClassDecl) {
  ...
  if (getLangOpts().F90 && ClassDecl->needsImplicitMoveConstructor()) {
  ...
  if (getLangOpts().F90 && ClassDecl->needsImplicitMoveAssignment()) {
...
bool Sema::CheckUsingShadowDecl(UsingDecl *Using, NamedDecl *Orig,
  ...
  if (!getLangOpts().F90 && CurContext->isRecord()) {
...
Decl *Sema::ActOnStartLinkageSpecification(Scope *S, SourceLocation ExternLoc,
  ...
  else if (Lang == "\"Fortran 90\"")
    Language = LinkageSpecDecl::lang_fortran90;
  else if (Lang == "\"Fortran 95\"")
    Language = LinkageSpecDecl::lang_fortran95;
  else {
...
FriendDecl *Sema::CheckFriendTypeDecl(SourceLocation LocStart,
  ...
  if (getLangOpts().F90 && LocStart != FriendLoc)
