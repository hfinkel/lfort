...
lfort - the LFort C, C++, and Objective-C compiler

=head1 SYNOPSIS

B<lfort> [B<-c>|B<-S>|B<-E>] B<-std=>I<standard> B<-g>
...
B<lfort> is a C, C++, and Objective-C compiler which encompasses preprocessing,
parsing, optimization, code generation, assembly, and linking.  Depending on
which high-level mode setting is passed, LFort will stop before doing a full
link.  While LFort is highly integrated, it is important to understand the
...
The B<lfort> executable is actually a small driver which controls the overall
...
LFort also supports the use of an integrated assembler, in which the code
...
The LFort compiler supports a large number of options to control each of these
stages.  In addition to compilation of code, LFort also supports other tools:

B<LFort Static Analyzer>

The LFort Static Analyzer is a tool that scans source code to try to find bugs
through code analysis.  This tool uses many parts of LFort and is built into the
same driver.
...
LFort fully supports cross compilation as an inherent part of its design.
Depending on how your version of LFort is configured, it may have support for
...
Specify that LFort should generate code for a specific processor family member
...
Generate debug information.  Note that LFort debug information works best at
...
through LFort compiled stack frames.  This is on by default in x86-64.
...
These options control how LFort prints out information about diagnostics (errors
and warnings).  Please see the LFort User's Manual for more information.
...
Do not search lfort's builtin directory for include files.
...
Maintained by the LFort / LLVM Team (L<http://lfort.llvm.org>).
