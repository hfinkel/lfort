// RUN: %lfort_cc1 -g %s -emit-llvm -o %t -fblocks
// RUN: grep "func.start" %t | count 4
// RUN: %lfort_cc1 -g %s -triple i386-unknown-unknown -emit-llvm -o %t -fblocks -fblock-introspection
// RUN: grep "v8@?0i4" %t | count 1
// RUN: %lfort_cc1 -g %s -triple i386-unknown-unknown -emit-llvm -o %t -fblocks
