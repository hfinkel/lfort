...
void test(enum Color color) {
  ...
  switch (c2) {
    ...
    // RUN: %lfort_cc1 -fsyntax-only -code-completion-at=%s:19:10 %s -o - | FileCheck -check-prefix=CC1 %s
    // CHECK-CC1: Blue
    ...
    // RUN: %lfort_cc1 -fsyntax-only -code-completion-at=%s:25:10 %s -o - | FileCheck -check-prefix=CC2 %s      
