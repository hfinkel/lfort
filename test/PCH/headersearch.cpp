...
// Generate the PCH:
// RUN: cd %t_orig && %lfort_cc1 -x c++ -emit-pch -o all.h.pch -Isub2 all.h
// RUN: cp -pR %t_orig %t_moved

// Check diagnostic with location in original source:
// RUN: %lfort_cc1 -include-pch all.h.pch -I%t_moved -I%t_moved/sub2 -Wpadded -emit-obj -o %t.o %s 2> %t.stderr
// RUN: grep 'struct orig_sub' %t.stderr

// Check diagnostic with 2nd location in original source:
// RUN: not %lfort_cc1 -DREDECL -include-pch all.h.pch -I%t_moved -I%t_moved/sub2 -emit-obj -o %t.o %s 2> %t.stderr
// RUN: grep 'void foo' %t.stderr

// Check diagnostic with instantiation location in original source:
// RUN: not %lfort_cc1 -DINSTANTIATION -include-pch all.h.pch -I%t_moved -I%t_moved/sub2 -emit-obj -o %t.o %s 2> %t.stderr
