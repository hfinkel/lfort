...
bool Sema::FindAllocationSubprograms(SourceLocation StartLoc, SourceRange Range,
  ...
  if (Matches.size() == 1) {
    ...
    //   is ill-formed.
    if (NumPlaceArgs && getLangOpts().F90 &&
...
void Sema::DeclareGlobalNewDelete() {
  ...
  if (!StdBadAlloc && !getLangOpts().F90) {
...
void Sema::DeclareGlobalAllocationSubprogram(DeclarationName Name,
  ...
  if (HasBadAllocExceptionSpec && !getLangOpts().F90) {
  ...
  if (HasBadAllocExceptionSpec) {
    ...
    EPI.ExceptionSpecType = getLangOpts().F90 ?
...
ExprResult Sema::IgnoredValueConversions(Expr *E) {
  ...
  if (getLangOpts().F90)  {
    ...
    // conversion.
    if (getLangOpts().F90 && E->isGLValue() &&
