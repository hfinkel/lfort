// RUN: %lfort_cc1 -emit-pch -o %t.1.ast %S/Inputs/var1.c
// RUN: %lfort_cc1 -emit-pch -o %t.2.ast %S/Inputs/var2.c
// RUN: %lfort_cc1 -ast-merge %t.1.ast -ast-merge %t.2.ast -fsyntax-only -fdiagnostics-show-note-include-stack %s 2>&1 | FileCheck %s
