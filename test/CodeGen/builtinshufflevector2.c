// RUN: %lfort -emit-llvm -S -o - %s | FileCheck %s

typedef float float4 __attribute__((ext_vector_type(4)));
typedef unsigned int uint4 __attribute__((ext_vector_type(4)));

// CHECK: define void @lfort_shufflevector_v_v(
void lfort_shufflevector_v_v( float4* A, float4 x, uint4 mask ) {
...
//
// Here is where ToT LFort code generation makes a mistake.  
...
// CHECK: define void @lfort_shufflevector_v_v_c(
void lfort_shufflevector_v_v_c( float4* A, float4 x, float4 y, uint4 mask ) {
