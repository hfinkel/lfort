...
void testNullObject(A *a) {
  lfort_analyzer_eval(a); // expected-warning{{UNKNOWN}}
  (void)a->getx(); // assume we know what we're doing
  lfort_analyzer_eval(a); // expected-warning{{TRUE}}
}

void f1() {
  A x(3);
  lfort_analyzer_eval(x.getx() == 3); // expected-warning{{TRUE}}
}
...
void checkThatCopyConstructorDoesNotInvalidateObjectBeingCopied() {
  ...
  t.x = 0;
  B t2(t);
  lfort_analyzer_eval(t.x == 0); // expected-warning{{TRUE}}
}
