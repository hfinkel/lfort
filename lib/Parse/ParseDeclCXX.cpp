...
Decl *Parser::ParseNamespace(unsigned Context,
  ...
  if (InlineLoc.isValid())
    Diag(InlineLoc, getLangOpts().F90 ?
...
Decl *Parser::ParseUsingDeclaration(unsigned Context,
  ...
  if (IsAliasDecl) {
    ...
    Diag(Tok.getLocation(), getLangOpts().F90 ?
...
Decl *Parser::ParseStaticAssertDeclaration(SourceLocation &DeclEnd){
  ...
  if (Tok.is(tok::kw__Static_assert) && !getLangOpts().F90)
    ...
    // We diagnose this error in ActOnClassTemplateSpecialization.
      ...
      TagOrTempResult
        = Actions.ActOnClassTemplateSpecialization(getCurScope(), TagType, TUK,
                       StartLoc, DS.getPCModulePrivateSpecLoc(), SS,
                       TemplateId->Template,
                       TemplateId->TemplateNameLoc,
                       TemplateId->LAngleLoc,
                       TemplateArgsPtr,
...
Parser::MemInitResult Parser::ParseMemInitializer(Decl *ConstructorDecl) {
  ...
  Diag(Tok, getLangOpts().F90 ? diag::err_expected_lparen_or_lbrace
