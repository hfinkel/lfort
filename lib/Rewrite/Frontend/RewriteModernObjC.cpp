...
namespace {
  ...
  public:
    ...
    Expr *SynthMsgSendStretCallExpr(SubprogramDecl *MsgSendStretFlavor,
...
void RewriteModernObjC::RewriteBlocksInSubprogramProtoType(QualType funcType,
                                                   NamedDecl *D) {
...
void RewriteModernObjC::CheckSubprogramPointerDecl(QualType funcType, NamedDecl *ND) {
...
ASTConsumer *lfort::CreateModernObjCRewriter(const std::string& InFile,
                                       raw_ostream* OS,
                                       DiagnosticsEngine &Diags,
...
Stmt *RewriteModernObjC::RewritePropertyOrImplicitSetter(PseudoObjectExpr *PseudoOp) {
  ...
  SmallVector<Expr*, 2> Args;
  {
    ...
    for (unsigned i = 0; i < numArgs; i++) {
      ...
      Arg = cast<Expr>(RewriteSubprogramBodyOrGlobalInitializer(Arg));
      Args.push_back(Arg);
    }
  }
...
CallExpr *RewriteModernObjC::SynthesizeCallToSubprogramDecl(
...
void RewriteModernObjC::SynthSelGetUidSubprogramDecl() {
...
void RewriteModernObjC::RewriteSubprogramDecl(SubprogramDecl *FD) {
...
void RewriteModernObjC::RewriteBlockLiteralSubprogramDecl(SubprogramDecl *FD) {
...
// SynthSuperContructorSubprogramDecl - id __rw_objc_super(id obj, id super);
void RewriteModernObjC::SynthSuperContructorSubprogramDecl() {
...
void RewriteModernObjC::SynthMsgSendSubprogramDecl() {
...
// SynthMsgSendSuperSubprogramDecl - id objc_msgSendSuper(void);
void RewriteModernObjC::SynthMsgSendSuperSubprogramDecl() {
...
void RewriteModernObjC::SynthMsgSendStretSubprogramDecl() {
...
void RewriteModernObjC::SynthMsgSendSuperStretSubprogramDecl() {
...
void RewriteModernObjC::SynthMsgSendFpretSubprogramDecl() {
...
// SynthGetClassSubprogramDecl - Class objc_getClass(const char *name);
void RewriteModernObjC::SynthGetClassSubprogramDecl() {
...
void RewriteModernObjC::SynthGetSuperClassSubprogramDecl() {
...
// SynthGetMetaClassSubprogramDecl - Class objc_getMetaClass(const char *name);
void RewriteModernObjC::SynthGetMetaClassSubprogramDecl() {
...
Stmt *RewriteModernObjC::RewriteObjCArrayLiteralExpr(ObjCArrayLiteral *Exp) {
  ...
  QualType NSArrayFType =
    getSimpleSubprogramType(Context->VoidTy, &IntQT, 1, true);
  std::string NSArrayFName("__NSContainer_literal");
  SubprogramDecl *NSArrayFD = SynthBlockInitSubprogramDecl(NSArrayFName);
...
Stmt *RewriteModernObjC::RewriteObjCDictionaryLiteralExpr(ObjCDictionaryLiteral *Exp) {
  ...
  SubprogramDecl *NSDictFD = SynthBlockInitSubprogramDecl(NSDictFName);
...
/// getSubprogramSourceLocation - returns start location of a function
/// definition. Complication arises when function has declared as
/// extern "C" or extern "C" {...}
static SourceLocation getSubprogramSourceLocation (RewriteModernObjC &R,
                                                 SubprogramDecl *FD) {
  ...
  if (FD->getStorageClassAsWritten() != SC_None)
    R.RewriteBlockLiteralSubprogramDecl(FD);
...
Expr *RewriteModernObjC::SynthMsgSendStretCallExpr(SubprogramDecl *MsgSendStretFlavor,
                                                 QualType msgSendType, 
                                                 QualType returnType, 
                                                 SmallVectorImpl<QualType> &ArgTypes,
                                                 SmallVectorImpl<Expr*> &MsgExprs,
  ...
  if (CurSubprogramDef)
    FunLocStart = getSubprogramSourceLocation(*this, CurSubprogramDef);
  else {
...
void RewriteModernObjC::InsertBlockLiteralsWithinSubprogram(SubprogramDecl *FD) {
  SourceLocation FunLocStart = 
    (!Blocks.empty()) ? getSubprogramSourceLocation(*this, FD)
...
bool RewriteModernObjC::convertObjCTypeToCStyleType(QualType &T) {
  QualType oldT = T;
  convertBlockPointerToSubprogramPointer(T);
  if (T->isSubprogramPointerType()) {
    QualType PointeeTy;
    if (const PointerType* PT = T->getAs<PointerType>()) {
      PointeeTy = PT->getPointeeType();
      if (const SubprogramType *FT = PointeeTy->getAs<SubprogramType>()) {
        T = convertSubprogramTypeOfBlocks(FT);
        T = Context->getPointerType(T);
      }
    }
  }
...
QualType RewriteModernObjC::convertSubprogramTypeOfBlocks(const SubprogramType *FT) {
...
void RewriteModernObjC::RewriteBlockPointerSubprogramArgs(SubprogramDecl *FD) {
...
Stmt *RewriteModernObjC::RewriteSubprogramBodyOrGlobalInitializer(Stmt *S) {
