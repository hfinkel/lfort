// RUN: %lfort_cc1 -analyze -analyzer-checker=core,unix.Malloc,debug.ExprInspection -verify %s
// REQUIRES: LP64
...
void vla(int n) {
  int nums[n];
  nums[0] = 1;
  lfort_analyzer_eval(nums[0] == 1); // expected-warning{{TRUE}}
  ...
  structs[0].x = 1;
  lfort_analyzer_eval(structs[0].x == 1); // expected-warning{{TRUE}}
}
