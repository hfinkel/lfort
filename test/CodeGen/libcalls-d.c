...
// RUN: %lfort_cc1 %s -emit-llvm -fno-math-errno -O0 -o - | grep {call.*exp2\\.f64}
// RUN: %lfort_cc1 %s -emit-llvm -fmath-errno -O0 -o - | grep {call.*exp2}
// RUN: %lfort_cc1 %s -emit-llvm -O1 -o - | grep {call.*ldexp}
// RUN: %lfort_cc1 %s -emit-llvm -O3 -fno-builtin -o - | grep {call.*exp2}
