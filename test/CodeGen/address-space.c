// RUN: %lfort_cc1 -triple x86_64-apple-darwin -emit-llvm < %s | FileCheck %s
// RUN: %lfort_cc1 -triple x86_64-apple-darwin -emit-llvm < %s | grep 'load.*addrspace(2).. @A'
// RUN: %lfort_cc1 -triple x86_64-apple-darwin -emit-llvm < %s | grep 'load.*addrspace(2).. @B'
