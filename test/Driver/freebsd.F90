...
! RUN: %lfort -no-canonical-prefixes \
! RUN:   -target powerpc-pc-freebsd8 %s    \
! RUN:   --sysroot=%S/Inputs/basic_freebsd_tree -### 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-PPC %s
! CHECK-PPC: lfort{{.*}}" "-cc1" "-triple" "powerpc-pc-freebsd8"
! CHECK-PPC: ld{{.*}}" "--sysroot=[[SYSROOT:[^"]+]]"
! CHECK-PPC: "--eh-frame-hdr" "-dynamic-linker" "{{.*}}ld-elf{{.*}}" "-o" "a.out" "{{.*}}crt1.o" "{{.*}}crti.o" "{{.*}}crtbegin.o" "-L[[SYSROOT]]/usr/lib" "{{.*}}.o" "-lfortrt" "-lm" "-lgcc" "--as-needed" "-lgcc_s" "--no-as-needed" "-lc" "-lgcc" "--as-needed" "-lgcc_s" "--no-as-needed" "{{.*}}crtend.o" "{{.*}}crtn.o"
!
! RUN: %lfort -no-canonical-prefixes \
! RUN:   -target powerpc64-pc-freebsd8 %s                              \
! RUN:   --sysroot=%S/Inputs/basic_freebsd64_tree -### 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-PPC64 %s
! CHECK-PPC64: lfort{{.*}}" "-cc1" "-triple" "powerpc64-pc-freebsd8"
! CHECK-PPC64: ld{{.*}}" "--sysroot=[[SYSROOT:[^"]+]]"
! CHECK-PPC64: "--eh-frame-hdr" "-dynamic-linker" "{{.*}}ld-elf{{.*}}" "-o" "a.out" "{{.*}}crt1.o" "{{.*}}crti.o" "{{.*}}crtbegin.o" "-L[[SYSROOT]]/usr/lib" "{{.*}}.o" "-lfortrt" "-lm" "-lgcc" "--as-needed" "-lgcc_s" "--no-as-needed" "-lc" "-lgcc" "--as-needed" "-lgcc_s" "--no-as-needed" "{{.*}}crtend.o" "{{.*}}crtn.o"
!
!
! Check that -m32 properly adjusts the toolchain flags.
!
! RUN: %lfort -no-canonical-prefixes -target x86_64-pc-freebsd8 -m32 %s \
! RUN:   --sysroot=%S/Inputs/multiarch_freebsd64_tree -### 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-LIB32 %s
! CHECK-LIB32: lfort{{.*}}" "-cc1" "-triple" "i386-pc-freebsd8"
! CHECK-LIB32: ld{{.*}}" {{.*}} "-m" "elf_i386_fbsd"
!
! RUN: %lfort -target x86_64-pc-freebsd8 -m32 %s 2>&1 \
...
! RUN: %lfort -no-canonical-prefixes -target x86_64-pc-freebsd9 -m32 %s \
! RUN:   --sysroot=%S/Inputs/multiarch_freebsd64_tree -### 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-LDFLAGS9 %s
! RUN: %lfort -no-canonical-prefixes -target x86_64-pc-freebsd10.0 -m32 %s \
! RUN:   --sysroot=%S/Inputs/multiarch_freebsd64_tree -### 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-LDFLAGS9 %s
...
! RUN: %lfort -no-canonical-prefixes -target x86_64-pc-freebsd8 -static %s \
! RUN:   --sysroot=%S/Inputs/multiarch_freebsd64_tree -### 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-STATIC %s
! CHECK-STATIC: crt1.o
! CHECK-STATIC: crtbeginT.o

! RUN: %lfort -no-canonical-prefixes -target x86_64-pc-freebsd8 -shared %s \
! RUN:   --sysroot=%S/Inputs/multiarch_freebsd64_tree -### 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-SHARED %s
! CHECK-SHARED: crti.o
! CHECK-SHARED: crtbeginS.o

! RUN: %lfort -no-canonical-prefixes -target x86_64-pc-freebsd8 -pie %s \
! RUN:   --sysroot=%S/Inputs/multiarch_freebsd64_tree -### 2>&1 \
...
! RUN: %lfort -no-canonical-prefixes -target x86_64-pc-freebsd8 %s \
! RUN:   --sysroot=%S/Inputs/multiarch_freebsd64_tree -### 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-NORMAL %s
! CHECK-NORMAL: crt1.o
! CHECK-NORMAL: crtbegin.o

! RUN: %lfort %s -### -o %t.o -target arm-unknown-freebsd10.0 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-ARM %s
! CHECK-ARM: lfort{{.*}}" "-cc1"{{.*}}" "-fsjlj-exceptions"
! CHECK-ARM: as{{.*}}" "-mfpu=softvfp"{{.*}}"-matpcs"

! RUN: %lfort %s -### -o %t.o -target arm-gnueabi-freebsd10.0 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-ARM-EABI %s
! CHECK-ARM-EABI-NOT: lfort{{.*}}" "-cc1"{{.*}}" "-fsjlj-exceptions"
