...
// RUN: %lfort_cc1 -DTEST4 -verify %s 2>&1 | FileCheck -check-prefix=CHECK4 %s
// RUN: %lfort_cc1 -DTEST5 -verify %s 2>&1 | FileCheck -check-prefix=CHECK5 %s
...
#if 0
// RUN: %lfort_cc1 -verify %t.invalid 2>&1 | FileCheck -check-prefix=CHECK6 %s
...
// RUN: echo -e '//expected-error@2{{1}}\n#error 2' | %lfort_cc1 -verify 2>&1 | FileCheck -check-prefix=CHECK7 %s
