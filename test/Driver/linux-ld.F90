...
! RUN: %lfort -target x86_64-unknown-linux -### %s \
! RUN:        --sysroot=%S/Inputs/basic_linux_tree 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-NOCRTFASTMATH %s
! RUN: %lfort -target x86_64-unknown-linux -### %s -ffast-math \
! RUN:        --sysroot=%S/Inputs/basic_linux_tree 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-CRTFASTMATH %s
! RUN: %lfort -target x86_64-unknown-linux -### %s -funsafe-math-optimizations\
! RUN:        --sysroot=%S/Inputs/basic_linux_tree 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-CRTFASTMATH %s
! RUN: %lfort -target x86_64-unknown-linux -### %s -ffast-math -fno-fast-math \
! RUN:        --sysroot=%S/Inputs/basic_linux_tree 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-NOCRTFASTMATH %s
! We don't have crtfastmath.o in the i386 tree, use it to check that file
! detection works.
! RUN: %lfort -target i386-unknown-linux -### %s -ffast-math \
! RUN:        --sysroot=%S/Inputs/basic_linux_tree 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-NOCRTFASTMATH %s
