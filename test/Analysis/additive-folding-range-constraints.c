// RUN: %lfort_cc1 -analyze -analyzer-checker=core,debug.ExprInspection -verify -analyzer-constraints=range %s
...
void smallAdjustmentGT (unsigned a) {
  if (a+2 > 1)
    lfort_analyzer_eval(a < UINT_MAX-1); // expected-warning{{TRUE}}
  else
    lfort_analyzer_eval(a == UINT_MAX-1 || a == UINT_MAX); // expected-warning{{TRUE}}
}
...
void smallAdjustmentLT (unsigned a) {
  if (a+1 < 2)
    lfort_analyzer_eval(a == 0 || a == UINT_MAX); // expected-warning{{TRUE}}
}

void smallAdjustmentLE (unsigned a) {
  if (a+1 <= 2)
    lfort_analyzer_eval(a == 0 || a == 1 || a == UINT_MAX); // expected-warning{{TRUE}}
}
...
void largeAdjustmentGE (unsigned a) {
  if (a-2 >= UINT_MAX-1)
    lfort_analyzer_eval(a == 1 || a == 0); // expected-warning{{TRUE}}
  else
    lfort_analyzer_eval(a > 1); // expected-warning{{TRUE}}
}
...
void mixedComparisons1(signed char a) {
  ...
  lfort_analyzer_eval((a - 2) >= (min + 5LL)); // expected-warning{{TRUE}}

  lfort_analyzer_eval(a == 0); // expected-warning{{UNKNOWN}}
  lfort_analyzer_eval(a == 0x7F); // expected-warning{{UNKNOWN}}
  lfort_analyzer_eval(a == -0x80); // expected-warning{{UNKNOWN}}
}

void mixedComparisons2(signed char a) {
  // Case 2: Only the lower end of the range is outside.
  lfort_analyzer_eval((a - 5) < (-0x81LL)); // expected-warning{{UNKNOWN}}

  if ((a - 5) < (-0x81LL)) {
    lfort_analyzer_eval(a == 0); // expected-warning{{FALSE}}
    lfort_analyzer_eval(a == 0x7F); // expected-warning{{FALSE}}
    lfort_analyzer_eval(a == -0x80); // expected-warning{{UNKNOWN}}
  }
}
...
void mixedComparisons3(signed char a) {
  ...
  lfort_analyzer_eval((a - 0x200) < -0x100LL); // expected-warning{{TRUE}}
...
void mixedComparisons4(signed char a) {
  ...
  lfort_analyzer_eval((a - 5) > 0LL); // expected-warning{{UNKNOWN}}

  if ((a - 5) > 0LL) {
    lfort_analyzer_eval(a == 0); // expected-warning{{FALSE}}
    lfort_analyzer_eval(a == 0x7F); // expected-warning{{UNKNOWN}}
    lfort_analyzer_eval(a == -0x80); // expected-warning{{FALSE}}
  }
}
...
void mixedComparisons9(signed char a) {
  ...
  lfort_analyzer_eval((a + 2) <= (max - 5LL)); // expected-warning{{TRUE}}
