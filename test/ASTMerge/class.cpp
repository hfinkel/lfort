// RUN: %lfort_cc1 -emit-pch -o %t.1.ast %S/Inputs/class1.cpp
// RUN: %lfort_cc1 -emit-pch -o %t.2.ast %S/Inputs/class2.cpp
// RUN: %lfort_cc1 -ast-merge %t.1.ast -ast-merge %t.2.ast -fsyntax-only %s 2>&1 | FileCheck %s
// RUN: %lfort_cc1 -ast-merge %t.1.ast -ast-merge %t.2.ast -fsyntax-only %s 2>&1 -Wno-odr | count 0
