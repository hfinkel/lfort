...
! CHECK-LD: ld{{.*}}" "-e" "__start" "--eh-frame-hdr" "-Bdynamic" "-dynamic-linker" "{{.*}}ld.so" "-o" "a.out" "{{.*}}crt0.o" "{{.*}}crtbegin.o" "{{.*}}.o" "-lfortrt" "-lm" "-lgcc" "-lc" "-lgcc" "{{.*}}crtend.o"

! RUN: %lfort -no-canonical-prefixes -target i686-pc-openbsd -pg -pthread %s -### 2>&1 \
! RUN:   | FileCheck --check-prefix=CHECK-PG %s
! CHECK-PG: lfort{{.*}}" "-cc1" "-triple" "i686-pc-openbsd"
! CHECK-PG: ld{{.*}}" "-e" "__start" "--eh-frame-hdr" "-Bdynamic" "-dynamic-linker" "{{.*}}ld.so" "-o" "a.out" "{{.*}}crt0.o" "{{.*}}crtbegin.o" "{{.*}}.o" "-lfortrt" "-lm_p" "-lgcc" "-lpthread_p" "-lc_p" "-lgcc" "{{.*}}crtend.o"
...
! CHECK-LD-R: ld{{.*}}" "-e" "__start" "--eh-frame-hdr" "-Bdynamic" "-dynamic-linker" "{{.*}}ld.so" "-o" "a.out" "{{.*}}crt0.o" "{{.*}}crtbegin.o" "-L{{.*}}" "-r" "{{.*}}.o" "-lfortrt" "-lm" "-lgcc" "-lc" "-lgcc" "{{.*}}crtend.o"
! CHECK-LD-S: lfort{{.*}}" "-cc1" "-triple" "i686-pc-openbsd"
! CHECK-LD-S: ld{{.*}}" "-e" "__start" "--eh-frame-hdr" "-Bdynamic" "-dynamic-linker" "{{.*}}ld.so" "-o" "a.out" "{{.*}}crt0.o" "{{.*}}crtbegin.o" "-L{{.*}}" "-s" "{{.*}}.o" "-lfortrt" "-lm" "-lgcc" "-lc" "-lgcc" "{{.*}}crtend.o"
! CHECK-LD-T: lfort{{.*}}" "-cc1" "-triple" "i686-pc-openbsd"
! CHECK-LD-T: ld{{.*}}" "-e" "__start" "--eh-frame-hdr" "-Bdynamic" "-dynamic-linker" "{{.*}}ld.so" "-o" "a.out" "{{.*}}crt0.o" "{{.*}}crtbegin.o" "-L{{.*}}" "-t" "{{.*}}.o" "-lfortrt" "-lm" "-lgcc" "-lc" "-lgcc" "{{.*}}crtend.o"
! CHECK-LD-Z: lfort{{.*}}" "-cc1" "-triple" "i686-pc-openbsd"
! CHECK-LD-Z: ld{{.*}}" "-e" "__start" "--eh-frame-hdr" "-Bdynamic" "-dynamic-linker" "{{.*}}ld.so" "-o" "a.out" "{{.*}}crt0.o" "{{.*}}crtbegin.o" "-L{{.*}}" "-Z" "{{.*}}.o" "-lfortrt" "-lm" "-lgcc" "-lc" "-lgcc" "{{.*}}crtend.o"
