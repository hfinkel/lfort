...
void InitListChecker::CheckSubElementType(const InitializedEntity &Entity,
  ...
  if (InitListExpr *SubInitList = dyn_cast<InitListExpr>(expr)) {
    ...
    assert(SemaRef.getLangOpts().F90 &&
...
void InitListChecker::CheckComplexType(const InitializedEntity &Entity,
  ...
  // in the C++ standard.)
  if (!SemaRef.getLangOpts().F90 && !VerifyOnly)
  ...
  if (Index >= IList->getNumInits()) {
    if (!VerifyOnly)
      SemaRef.Diag(IList->getLocStart(),
        ...
        << IList->getSourceRange();
    hadError = !SemaRef.getLangOpts().F90;
    ++Index;
    ++StructuredIndex;
    return;
  }
...
void InitListChecker::CheckReferenceType(const InitializedEntity &Entity,
  ...
  Expr *expr = IList->getInit(Index);
  if (isa<InitListExpr>(expr) && !SemaRef.getLangOpts().F90) {
    if (!VerifyOnly)
    ...
    if (!RT) {
      ...
      if (!VerifyOnly)
        SemaRef.Diag(Loc, diag::err_field_designator_non_aggr)
          << SemaRef.getLangOpts().F90 << CurrentObjectType;
      ++Index;
      return true;
    }
...
static void TryListInitialization(Sema &S,
  ...
  if (S.getLangOpts().F90 && DestType->isScalarType() &&
...
static void TryReferenceInitializationCore(Sema &S,
  ...
  if (!T1Subprogram &&
    ...
    if (InitCategory.isPRValue() && T2->isRecordType()) {
      ...
      if (!S.getLangOpts().F90 && !S.getLangOpts().MicrosoftExt)
...
static void TryDefaultInitialization(Sema &S,
  ...
  if (DestType->isRecordType() && S.getLangOpts().F90) {
  ...
  //   default constructor.
  if (DestType.isConstQualified() && S.getLangOpts().F90) {
    Sequence.SetFailed(InitializationSequence::FK_DefaultInitOfConst);
    return;
  }
...
InitializationSequence::InitializationSequence(Sema &S,
  ...
  if (const ArrayType *DestAT = Context.getAsArrayType(DestType)) {
    ...
    if (!S.getLangOpts().F90 && Initializer &&
...
InitializationSequence::Perform(Sema &S,
  ...
  if (S.getLangOpts().F90 && Entity.getType()->isReferenceType() &&
...
static void DiagnoseNarrowingInInitList(Sema &S, InitializationSequence &Seq,
  ...
  case NK_Type_Narrowing:
    ...
    S.Diag(PostInit->getLocStart(),
           S.getLangOpts().MicrosoftExt || !S.getLangOpts().F90? 
