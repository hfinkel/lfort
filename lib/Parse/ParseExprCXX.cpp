...
ExprResult Parser::ParseLambdaExpressionAfterIntroducer(
  ...
  // it.
  unsigned ScopeFlags = Scope::BlockScope | Scope::SubPgmScope | Scope::DeclScope;
...
Parser::ParseCXXTypeConstructExpression(const DeclSpec &DS) {
  ...
  assert((Tok.is(tok::l_paren) ||
          (getLangOpts().F90 && Tok.is(tok::l_brace)))
         && "Expected '(' or '{'!");
  ...
  switch (Tok.getKind()) {
    ...
    case tok::kw_delete: {
      ...
      if (Tok.is(tok::l_square) &&
          (!getLangOpts().F90 || NextToken().isNot(tok::l_square))) {
        // Consume the '[' and ']'.
        BalancedDelimiterTracker T(*this, tok::l_square);
        T.consumeOpen();
        T.consumeClose();
...
#include "lfort/Basic/OperatorKinds.def"
  ...
  if (getLangOpts().F90 && isTokenStringLiteral()) {
...
Parser::ParseCXXNewExpression(bool UseGlobal, SourceLocation Start) {
  ...
  if (Tok.is(tok::l_paren)) {
    ...
    Initializer = Actions.ActOnParenListExpr(ConstructorLParen,
                                             ConstructorRParen,
                                             ConstructorArgs);
  } else if (Tok.is(tok::l_brace) && getLangOpts().F90) {
    Diag(Tok.getLocation(),
         diag::warn_cxx98_compat_generalized_initializer_lists);
